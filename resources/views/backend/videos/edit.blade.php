...
@section('title','CoolPad || video Edit')
...
<div class="card">
    ...
    <div class="card-body">
      <form method="post" action="{{route('video.update',$video->id)}}">
        ...
        <div class="form-group">
          <label for="inputVideo_url" class="col-form-label">video_url <span class="text-danger">*</span></label>
        <input id="inputVideo_url" type="text" name="video_url" placeholder="Enter video_url"  value="{{$video->video_url}}" class="form-control">
        @error('video_url')
        <span class="text-danger">{{$message}}</span>
        @enderror
        </div>
        ...
        <div class="input-group">
          ...
          <input id="thumbnail" class="form-control" type="text" name="image" value="{{$video->image}}">
        </div>
...
<script>
    ...
    $('#video_url').summernote({
      placeholder: "Write short video_url.....",
        tabsize: 2,
        height: 150
    });
    });
